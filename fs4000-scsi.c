/*--------------------------------------------------------------
                ...
                FS4000 SCANNER ACCESS CODE

        Routines for Canon FS4000US scanner access

  Copyright (C) 2004  Dave Burns
  ...
  MA 02111-1307, USA.
        ...
        2004-08-10      Release by Dave Burns.

        2004-12-03      Changes by Steven Saunderson.
...
int fs4000_test_unit_ready(void)
{
    ...
    return fs4000_do_scsi(&cdb, sizeof(cdb), 0, 0, 0);
}
...
int fs4000_inquiry(FS4000_INQUIRY_DATA_IN *data)
{
    ...
    result = fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_IN, &int_data, sizeof(int_data));
    if (result == 0)
        if (data != NULL)
...
int fs4000_mode_select(void)
{
    ...
    return fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_OUT, &data, sizeof(data));
}
                      ...
                      BYTE bits_per_pixel)
{
  ...
  result = fs4000_get_window_rec (&data);
  if (result)
    return result;
                      ...
                      BYTE *bits_per_pixel)
{
    ...
    result = fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_OUT, &data, sizeof(data));
    if (result == 0) {
...
int fs4000_read(int buffer_size, PIXEL *buffer)
{
    ...
    return fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_IN, buffer, buffer_size);
}
...
int fs4000_get_data_status(UINT4 *num_scanlines, UINT4 *num_bytes_per_scanline)
{
    ...
    result = fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_IN, &data, sizeof(data));
    if (result == 0) {
                         ...
                         FS4000_GET_SCAN_MODE_DATA_IN_12 *data_12)
{
    ...
    if (rare) {
        ...
        result = fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_IN, &int_data_12, sizeof(int_data_12));
        if (result == 0) {
        ...
        result = fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_IN, &int_data_38, sizeof(int_data_38));
        if (result == 0) {
...
int
fs4000_scan_for_thumbnail       (void)
{
  ...
  if (fs4000_debug == 1) print_FS4000_SCAN_FOR_THUMBNAIL_CDB (&cdb);

  return fs4000_do_scsi (&cdb, sizeof (cdb), SRB_DIR_OUT, &pdb, 1);
}
                           ...
                           BYTE *film_holder, BYTE *num_frames, UINT2 *film_position, BYTE *focus_position)
{
    ...
    result = fs4000_do_scsi(&cdb, sizeof(cdb), SRB_DIR_IN, &data, shorter ? sizeof(FS4000_GET_FILM_STATUS_DATA_IN_25) : sizeof(data));
    if (result == 0) {
...
void create_FS4000_SET_WINDOW_FOR_THUMBNAIL_CDB(FS4000_SET_WINDOW_FOR_THUMBNAIL_CDB *cdb) {
    memset(cdb, 0, sizeof(*cdb));
    cdb->opcode = 0xf0;
    cdb->data_length = 72;
}
...
void create_FS4000_GET_WINDOW_FOR_THUMBNAIL_CDB(FS4000_GET_WINDOW_FOR_THUMBNAIL_CDB *cdb) {
    memset(cdb, 0, sizeof(*cdb));
    cdb->opcode = 0xf1;
    cdb->flags = 0x01;
    cdb->data_length = 72;
...
void free_rgb_deinterleave(void)
{
    unsigned int i;
...
int
fs4000_set_thumbnail_window     (UINT2 x_res,
                                 UINT2 y_res,
                                 UINT4 x_upper_left,
                                 UINT4 y_upper_left,
                                 UINT4 width,
...
int
fs4000_get_thumbnail_window     (UINT2 *x_res,
                                 UINT2 *y_res,
                                 UINT4 *x_upper_left,
                                 UINT4 *y_upper_left,
                                 UINT4 *width,
                                 ...
                                 BYTE *bits_per_pixel)
{
  ...
  if (fs4000_debug == 1) print_fs4000_get_thumbnail_window(&cdb, NULL);
  ...
  if (result == 0)
    {
    swap_endian_FS4000_GETSET_WINDOW_DATA(&data);
    if (fs4000_debug == 1) print_fs4000_get_thumbnail_window(NULL, &data);
...
int
fs4000_no_scsi          (void         * cdb         ,
                         unsigned int   cdb_length  ,
                         int            data_dir    ,
                         void         * data_buf    ,
                         unsigned int   data_buf_len)
{
  printf ("No transport for fs4000 SCSI I/O\n");
  exit (1);
  return 1;
}
...
int
fs4000_get_film_status_rec      (FS4000_GET_FILM_STATUS_DATA_IN_28 *data)
{
        FS4000_GET_FILM_STATUS_CDB      cdb;
        int                             result;
  ...
  if (result == 0)
    ...
    swap_endian_FS4000_GET_FILM_STATUS_DATA_IN ((FS4000_GET_FILM_STATUS_DATA_IN_25 *) data);
    if (fs4000_debug == 1)
      print_fs4000_get_film_status (NULL, NULL, data);
    }
...
int
fs4000_get_lamp_rec     (FS4000_GET_LAMP_DATA_IN *data)
{
        FS4000_GET_LAMP_CDB     cdb;
        int                     result;
...
int
fs4000_get_scan_mode_rec        (FS4000_GET_SCAN_MODE_DATA_IN_38 *data)
{
        FS4000_GET_SCAN_MODE_CDB        cdb;
        int                             result;
  ...
  if (result == 0)
    ...
    swap_endian_FS4000_GETSET_SCAN_MODE_DATA (data);
    if (fs4000_debug == 1)
      print_fs4000_get_scan_mode (NULL, data, NULL);
    }
...
int
fs4000_put_scan_mode_rec        (FS4000_DEFINE_SCAN_MODE_DATA_OUT *param)
{
        FS4000_DEFINE_SCAN_MODE_CDB             cdb;
        FS4000_DEFINE_SCAN_MODE_DATA_OUT        data = *param;
  ...
  if (fs4000_debug == 1)
    print_fs4000_define_scan_mode (&cdb, &data);
  swap_endian_FS4000_DEFINE_SCAN_MODE_CDB (&cdb);
  swap_endian_FS4000_GETSET_SCAN_MODE_DATA (&data);
  return fs4000_do_scsi (&cdb, sizeof (cdb), SRB_DIR_OUT, &data, sizeof (data));
}
...
int
fs4000_get_window_rec   (FS4000_GET_WINDOW_DATA_IN *data)
{
        FS4000_GET_WINDOW_CDB   cdb;
        int                     result;
...
int
fs4000_put_window_rec   (FS4000_SET_WINDOW_DATA_OUT *param)
{
        FS4000_SET_WINDOW_CDB           cdb;
        FS4000_SET_WINDOW_DATA_OUT      data = *param;
